/*Cadastro simples
Crie um programa que:
cadastre até 5 pessoas (struct)
nome (string)
idade (int)
Liste:
todas
apenas maiores de idade
?? Foco: vetor, struct, for, if.*/

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <ctype.h>
#include <locale.h>

typedef struct{
    char nome[100];
    int idade;
} Cadastro;

// Função utilitária para converter string para maiúsculas
void maiuscula(char *str){
    for(int i = 0; str[i] != '\0'; i++){
        str[i] = toupper((unsigned char)str[i]);
    }
}

// Função para criar uma pausa na tela (UX - Experiência do Usuário)
void continuar(){
    printf("\nPressione Enter para continuar...");
    // Loop para garantir que o buffer esteja limpo antes de esperar o Enter
    while(getchar() != '\n'); 
    printf("\n");
}

void preencher_dados(Cadastro *pessoas, int qtd){
    printf("\n\nDados dos alunos.\n");
    for(int i = 0; i < qtd; i++){
        int entrada_valida = 0;
        int nome_valido;
        
        // Validação do Nome
        do {
            nome_valido = 1;
            printf("\nInforme o %dº nome: ", i + 1);
            fgets(pessoas[i].nome, sizeof(pessoas[i].nome), stdin);
            pessoas[i].nome[strcspn(pessoas[i].nome, "\n")] = '\0'; // Remove o \n
            maiuscula(pessoas[i].nome);
            
            if(!strlen(pessoas[i].nome)) {
                printf("Erro: Nome não pode estar vazio!\n");
                nome_valido = 0;
            } else {
                // Garante que só tenha letras e espaços
                for(int j = 0; pessoas[i].nome[j] != '\0'; j++){
                    if(!isalpha((unsigned char)pessoas[i].nome[j]) && !isspace((unsigned char)pessoas[i].nome[j])){
                        nome_valido = 0;
                        printf("Erro: O nome deve conter apenas letras e espacos.\n");
                        break;
                    }
                }
            }
        } while(!nome_valido);
        
        // Validação da Idade
        while(!entrada_valida){
            printf("Idade do aluno: ");
            if(scanf("%d", &pessoas[i].idade) != 1){
                printf("Digite apenas numeros\n");
                while(getchar() != '\n'); // Limpa buffer
            } else if(pessoas[i].idade < 4){
                printf("Idade mínima 4 anos.\n");
            } else {
                entrada_valida = 1;
                while(getchar() != '\n'); // Limpa buffer pós-número
            }
        }
    }
}

// Exibe a lista completa. Note que ele chama continuar() no final.
void exibir_todos(Cadastro *pessoas, int qtd){
    printf("\n\n<<- RELATORIO DE ALUNOS ->>\n");
    printf("----------------------------------------------------------\n");
    printf("INDICE | NOME                                     | IDADE\n");
    printf("----------------------------------------------------------\n");
    for(int i = 0; i < qtd; i++){
        printf("%02d     | %-40s | %d anos\n", i+1, pessoas[i].nome, pessoas[i].idade);
        printf("----------------------------------------------------------\n");
    }
    continuar(); // Pausa antes de voltar ao menu/fluxo
}

// Filtra e exibe apenas maiores de 18 anos
void exibir_maior_idade(Cadastro *pessoas, int qtd){
    printf("\n\n<<- RELATORIO DE ALUNOS MAIORES DE IDADE ->>\n");
    printf("----------------------------------------------------------\n");
    printf("INDICE | NOME                                     | IDADE\n");
    printf("----------------------------------------------------------\n");
    for(int i = 0; i < qtd; i++){
        if(pessoas[i].idade >= 18){ // Filtro lógico
            printf("%02d     | %-40s | %d anos\n", i+1, pessoas[i].nome, pessoas[i].idade);
            printf("----------------------------------------------------------\n");
        }
    }
    continuar();
}

// Ordena ALFABETICAMENTE e exibe
// ATENÇÃO: Esta função altera a posição dos elementos no vetor original!
void exibir_ordenado(Cadastro *pessoas, int qtd){
    Cadastro aux; // Variável auxiliar para a troca (swap)
    
    // Algoritmo Bubble Sort
    for(int i = 0; i < qtd; i++){
        for(int j = 0; j < qtd - 1; j++){
            // strcmp > 0 significa que a string 'j' vem depois da 'j+1' no alfabeto
            if(strcmp(pessoas[j].nome, pessoas[j +1].nome) > 0){
                // Troca as posições
                aux = pessoas[j];
                pessoas[j] = pessoas[j + 1];
                pessoas[j + 1] = aux;
            }
        }
    }
    
    printf("\n\n<<- RELATORIO DE ALUNOS POR ORDEM ALFABETICA ->>\n");
    printf("----------------------------------------------------------\n");
    printf("INDICE | NOME                                     | IDADE\n");
    printf("----------------------------------------------------------\n");
    for(int i = 0; i < qtd; i++){
        printf("%02d     | %-40s | %d anos\n", i+1, pessoas[i].nome, pessoas[i].idade);
        printf("----------------------------------------------------------\n");
    }
    // Faltou chamar continuar() aqui se quiser manter o padrão
}

// Ordena por IDADE (Crescente) e exibe
// ATENÇÃO: Esta função também altera o vetor original!
void ordenar_por_idade(Cadastro *pessoas, int qtd) {
    Cadastro aux;
    for (int i = 0; i < qtd; i++) {
        for (int j = 0; j < qtd - 1; j++) {
            // Se a idade atual for maior que a próxima, troca
            if (pessoas[j].idade > pessoas[j + 1].idade) {
                aux = pessoas[j];
                pessoas[j] = pessoas[j + 1];
                pessoas[j + 1] = aux;
            }
        }
    }
    printf("\n\n<<- RELATORIO DE ALUNOS POR IDADE (CRESCENTE) ->>\n");
    printf("----------------------------------------------------------\n");
    printf("INDICE | NOME                                     | IDADE\n");
    printf("----------------------------------------------------------\n");

    for (int i = 0; i < qtd; i++) {
        printf("%02d     | %-40s | %d anos\n", i + 1, pessoas[i].nome, pessoas[i].idade);
        printf("----------------------------------------------------------\n");
    }
}

int main(){
    setlocale(LC_ALL, "Portuguese_Brazil");
    Cadastro *pessoas;
    int qtd, entrada_valida = 0;
    
    // Validação da quantidade inicial
    while(!entrada_valida){
        printf("\nQuantos alunos deseja cadastrar: ");
        if(scanf("%d", &qtd) != 1) {
            printf("Erro: Digite apenas números!\n");
        } else if(qtd < 1) {
            printf("Erro: Opção deve ser maior que 0!\n");
        } else {
            entrada_valida = 1;
        }
        while(getchar() != '\n');
    }
    
    // Alocação dinâmica
    pessoas = (Cadastro *)malloc(qtd * sizeof(Cadastro));
    if(pessoas == NULL){
        printf("Erro ao alocar memoria");
        return 1;
    }
      
    preencher_dados(pessoas, qtd);
    
    // Sequência de relatórios
    exibir_todos(pessoas, qtd);        // Mostra na ordem de cadastro
    exibir_maior_idade(pessoas, qtd);  // Filtra
    exibir_ordenado(pessoas, qtd);     // Ordena por NOME e mostra
    ordenar_por_idade(pessoas, qtd);   // Ordena por IDADE e mostra
    
    free(pessoas); // Libera memória
    return 0;
}

...
void validar_opcao(int *opcao){
    ...
    while(!entrada_valida) {
        printf("\nInforme o n?mero da op??o desejada (0-9): ");
        if(scanf("%d", opcao) != 1) {
            printf("Erro: Digite apenas n?meros!\n");
        } else if(*opcao < 0 || *opcao > 9) {
            printf("Erro: Op??o deve estar entre 0 e 9!\n");
        } else {
            entrada_valida = 1;
        }
        while(getchar() != '\n');
...
void cadastrar_filmes(Filme *filmes, int *total_filmes){
    ...
    if(*total_filmes >= TAM_MAX){
        printf("N?o ? poss?vel cadastrar filmes.\nLimite atingido.\n");
        continuar();
        return;
    }
    ...
    while(sair != 'n' && i < TAM_MAX){
        do {
            ...
            if(strlen(filmes[i].nome) == 0) {
                printf("Erro: Nome n?o pode estar vazio!\n");
            }
        ...
        if(i >= TAM_MAX) {
            printf("\nLimite m?ximo de %d filmes atingido!\n", TAM_MAX);
            printf("=====================================\n");
            continuar();
            return;
        }
...
void listar_filmes(Filme filmes[], int total_filmes){
    ...
    for(int i = 0; i < total_filmes; i++){
        ...
        printf("    Status: %s\n", filmes[i].status == 'D' ? "Dispon?vel" : "Locado");
...
void filmes_locados(Filme filmes[], int total_filmes){
    ...
    if(!encontrou){
        printf("Nenhum filme est? locado no momento.\n");
    }
    continuar();
}
...
void alugar_filme(Filme filmes[], int total_filmes){
    ...
    while(sair != 'n'){
        ...
        while(!entrada_valida){
            printf("\nInforme o n?mero do filme (1-%d): ", disponiveis);
            ...
            if(scanf("%d", &escolha) != 1){
                ...
                while(getchar() != '\n');
                printf("Erro: N?mero inv?lido! Escolha entre 1 e %d.\n", disponiveis);
            }
            else{
                entrada_valida = 1;
                while(getchar() != '\n');
...
void devolver_filme(Filme filmes[], int total_filmes){
    ...
    while(!entrada_valida){
        printf("\nInforme o n?mero do filme a devolver (1-%d): ", locados);
...
void filme_mais_locado(Filme filmes[], int total_filmes){
    ...
    if(max_locado == 0){
        ...
        printf("Status: %s\n", filmes[indice_mais_locado].status == 'D' ? "Dispon?vel" : "Locado");
        
        // Verificar se h? empate
        printf("\nOutros filmes com %d loca??es:\n", max_locado);
        int tem_empate = 0;
        for(int i = 0; i < total_filmes; i++){
...
void atualizar_filme(Filme filmes[], int total_filmes){
    ...
    while(!entrada_valida){
        printf("\nInforme o n?mero do filme (1-%d): ", total_filmes);
    ...
    printf("Status atual: %s\n", filmes[indice].status == 'D' ? "Dispon?vel" : "Locado");
